...
import com.example.jobservice.BuildConfig;
...
public class MainService extends Service {
    ...
    /**
     * 每1分钟检查一次链接状态，确保service不被杀掉
     */
    private static final int KEEP_ALIVE_INTERVAL = 5 * 1000;
    ...
    private void addAliveAlarm() {
        Log.e("MainService", "addAliveAlarm");
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            JobInfo.Builder _Builder = new JobInfo.Builder(1, new ComponentName(getApplication(), JobSchedulerService.class));
            if (Build.VERSION.SDK_INT >= 24) {
                Log.e("MainService", "Build.VERSION.SDK_INT");
                _Builder.setMinimumLatency(KEEP_ALIVE_INTERVAL); //执行的最小延迟时间
                _Builder.setOverrideDeadline(KEEP_ALIVE_INTERVAL);  //执行的最长延时时间
                _Builder.setBackoffCriteria(KEEP_ALIVE_INTERVAL, JobInfo.BACKOFF_POLICY_LINEAR);//线性重试方案
            } else {
                _Builder.setPeriodic(KEEP_ALIVE_INTERVAL);
            }
            _Builder.setRequiredNetworkType(JobInfo.NETWORK_TYPE_ANY);
            _Builder.setRequiresCharging(false); // 当插入充电器，执行该任务
